 ...
 *   node scripts/fi.ts --gleak 5 --cm 120 --steps 0,50,100,150,200
...
const { values: a } = parseArgs({
  options: {
    ...
    gleak: { type: "string" }, cm: { type: "string" }, ihold: { type: "string" },
...
if (a.help) {
  console.log("usage: node scripts/fi.ts [--model rs|gnrh] [--gleak nS] [--cm pF] [--ihold pA] [--rs MOhm] [--steps a,b,c] [--step-start ms] [--step-dur ms] [--sweep ms] [--glu r,g,tr,td] [--gaba r,g,tr,td,E] [--regular] [--seed n] [--rheobase] [--rec-noise mV] [--mem-noise sigma_pA,tau_ms]");
  process.exit(0);
}
...
const cellP = { ...publishedParams(model.id), ...(a.gleak ? { gLeak: +a.gleak } : {}), ...(a.cm ? { cm: +a.cm } : {}) };
...
console.log(`# leak ${f(cell.gLeak)} nS, Rin at rest ${f(cell.rinRest)} MOhm (measured), Cm ${cellP.cm} pF, Ihold ${inputs.ihold} pA, rest ${f(cell.vRest)} mV, Rin at Ihold ${f(cellRinAt(cell, inputs.ihold))} MOhm`);
