...
import { buildCell, cellRinAt, PUBLISHED_RS } from "../src/core/cell.ts";
...
const { values: a } = parseArgs({
  options: {
    rin: { type: "string" }, cm: { type: "string" }, ihold: { type: "string", default: "0" },
...
if (a.help) {
  console.log("usage: node scripts/fi.ts [--rin MOhm] [--cm pF] [--ihold pA] [--rs MOhm] [--steps a,b,c] [--step-start ms] [--step-dur ms] [--sweep ms] [--glu r,g,tr,td] [--gaba r,g,tr,td,E] [--regular] [--seed n] [--rheobase]");
  process.exit(0);
}
...
const cellP = { ...PUBLISHED_RS, ...(a.rin ? { rin: +a.rin } : {}), ...(a.cm ? { cm: +a.cm } : {}) };
const cell = buildCell(cellP);
const inputs = { ihold: +a.ihold!, electrode: { rs: +a.rs!, cp: 0, bridge: 1 }, glu: syn(DEFAULT_GLU, a.glu), gaba: syn(DEFAULT_GABA, a.gaba) };
...
console.log(`# gaba_excites CLI — Pospischil 2008 RS cell (ModelDB 123623)`);
...
if (a.rheobase) {
  const st = settle(cell, inputs.electrode, inputs.ihold, opt.settleMs, opt.dt);
