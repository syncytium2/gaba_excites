...
The cell is the regular-spiking pyramidal neuron of Pospischil et al. (2008),
...
On the published cell, with clock-like GABA (200 Hz, 3 nS, 0.5/10 ms),
...
- `src/core/` — the model, pure TypeScript, no DOM. `channels.ts` (the stock
  channels, behind a `Channel` interface), `cell.ts` (whole-cell ↔ density
  mapping, the Rin solve), `synapses.ts`, `simulate.ts` (electrode circuit and
  integrator), `analysis.ts` (spikes, threshold, F–I, Rin), `protocol.ts`
  (step lists and presets).
...
Cite the model: Pospischil M, et al. (2008) Minimal Hodgkin–Huxley type models
for different classes of cortical and thalamic neurons. *Biol Cybern*
99:427–441. [doi:10.1007/s00422-008-0263-8](https://doi.org/10.1007/s00422-008-0263-8).
